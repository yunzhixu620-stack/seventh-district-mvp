...
export function IntelBoard() {
  ...
  const messages = session.messages.filter((message) =>
    tab === 'public' ? message.channel !== 'private' : message.channel === 'private',
  )
        ...
        <div className="message-list">
          {messages.map((message) => (
            <article key={message.id}>
              <header>
                <strong>{message.sender === 'district' ? t('incident') : localize(npcProfiles[message.sender].callSign, language)}</strong>
                <time>{String(22 + Math.floor(message.round / 4)).padStart(2, '0')}:{String(40 + message.round * 2).padStart(2, '0')}</time>
              </header>
