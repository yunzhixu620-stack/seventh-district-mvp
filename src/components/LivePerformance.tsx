    ...
    <section className="performance-box" aria-live="polite">
      ...
      {actorLoading ? (
        ...
        <>
          ...
          <footer>
            <span>
              {localize(ui.subtext, language)}: {performance.subtext}
            </span>
          </footer>
        </>
      ) : (
        ...
        <div className="effect-strip">
          ...
          {effect.riskDelta !== 0 && (
            <span className={effect.riskDelta > 0 ? 'risk-up' : 'risk-down'}>
              {localize(
                effect.riskDelta > 0 ? ui.riskRise : ui.riskEase,
                language,
              )}{' '}
              {effect.riskDelta > 0 ? '+' : ''}
              {effect.riskDelta}
          ...
          {effect.spentComposure > 0 && (
            <span>
              {localize(ui.spentComposure, language)} -{effect.spentComposure}
            </span>
          )}
          {effect.spentFavor > 0 && (
            <span>
              {localize(ui.spentFavor, language)} -{effect.spentFavor}
            </span>
          )}
          {effect.addedClueIds.length > 0 && (
